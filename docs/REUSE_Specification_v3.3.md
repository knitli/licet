...
REUSE Tool — helper tool for compliance with this specification; available at https://codeberg.org/fsfe/reuse-tool.
...
License File — a file containing the text of a license, as defined in License Files.
...
Covered File — a file which must contain Licensing Information, as defined in Covered and ignored Files.
...
SPDX Specification — SPDX specification, version 2.3; as available at https://spdx.org/specifications.
...
DEP5 — Machine-readable Debian/copyright file, Version 1.0; as available at https://www.debian.org/doc/packaging-manuals/copyright-format/1.0/ and further defined in DEP5 (deprecated). Where the REUSE Specification and DEP5 state different things, the REUSE Specification takes precedence.

TOML — A configuration file format as available at https://toml.io/en/v1.0.0.
...
COPYING, LICENSE, and LICENCE. These MAY also contain a dash or dot separator followed by additional metadata, like LICENSE-MIT, COPYING.GPL, or LICENCE.md. These files SHOULD be in the root of the Project.
...
You MAY include COPYING or LICENSE files in your project for compliance with other standards, conventions, or tools. These files MAY contain a copy of the license text, a summary of your licensing, or anything else. These files are ignored by the REUSE Tool.
...
```python
 ...
 # SPDX-FileCopyrightText: 2019 Example Organization
...
Sometimes Licensing Information only applies to a certain Snippet instead of the whole Commentable File. You SHOULD use SPDX snippets to license these Snippets. When implementing SPDX snippets, you MUST use SPDX snippet tags (as defined in SPDX Specification, Annex H). This means that Copyright Notices inside of Snippets MUST be prefixed with SPDX-SnippetCopyrightText.
...
A JSON schema of version 1 of REUSE.toml is provided at https://reuse.software/reuse-toml-v1.schema.json for your convenience.
...
The Files tag MUST be followed by one or more paths to which the paragraph’s Licensing Information is applied.

The License tag MUST be followed by a valid SPDX License Expression describing the licensing of the associated files.

The Copyright tag MUST be followed by one or more Copyright Notices. The prefixes of the Copyright Notices MAY be omitted.
