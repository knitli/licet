...
- Q: When a file's in-file SPDX header and an out-of-band entry (`REUSE.toml`/`.reuse/dep5`) disagree on the license, which is authoritative for detection? → A: The **out-of-band entry wins** for the *detected actual* license. The declarative config (`license.toml`) remains the source of truth for *intent*; out-of-band only takes precedence over the in-file header when reading a file's actual state. A non-failing diagnostic records the override. (Note: the maintainer considers `REUSE.toml` poorly suited to declarative authoring — it is read for interop/detection only, never the authoring surface.)
...
- **Binary / non-annotatable files**: Files that cannot carry an inline header (images, binaries) MUST be handled through the REUSE-compatible out-of-band mechanism rather than skipped, so they still satisfy enforcement.
...
- **FR-003a**: When detecting a file's actual license, the system MUST consider both in-file SPDX headers and out-of-band REUSE entries (`REUSE.toml` / `.reuse/dep5`); licensing information from either source satisfies the declared intent for drift purposes. When the two sources disagree on the license for the same file, the out-of-band entry is authoritative for the detected actual license, and the system MUST emit a non-failing diagnostic that the in-file header was overridden.
...
- **FR-015**: The system MUST handle non-annotatable/binary files via the REUSE-compatible out-of-band mechanism so they are still covered by enforcement.
